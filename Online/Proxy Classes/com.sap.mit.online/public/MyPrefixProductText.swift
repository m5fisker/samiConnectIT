...
open class MyPrefixProductText: EntityValue {
    ...
    open class func array(from: EntityValueList) -> [MyPrefixProductText] {
        return ArrayConverter.convert(from.toArray(), [MyPrefixProductText]())
    }

    open func copy() -> MyPrefixProductText {
        return CastRequired<MyPrefixProductText>.from(copyEntity())
    }

    open var id: Int64? {
        get {
            return LongValue.optional(optionalValue(for: MyPrefixProductText.id))
        }
        set(value) {
            setOptionalValue(for: MyPrefixProductText.id, to: LongValue.of(optional: value))
        }
    }
    ...
    open var language: String? {
        get {
            return StringValue.optional(optionalValue(for: MyPrefixProductText.language))
        }
        set(value) {
            setOptionalValue(for: MyPrefixProductText.language, to: StringValue.of(optional: value))
        }
    }
